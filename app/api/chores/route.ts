...
export async function POST(request: NextRequest) {
  const body = await request.json() as { action?:string; id?:number; eventId?:number; person?:string; name?:string; category?:string; icon?:string; intervalDays?:number; points?:number; paused?:boolean; scheduleMode?:string; cadenceHours?:number; priority?:number; dueTime?:string|null };
