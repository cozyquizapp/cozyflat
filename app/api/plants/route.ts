...
export async function POST(request:NextRequest) {
  if (request.headers.get('content-type')?.includes('multipart/form-data')) {
    ...
    await addPlant(String(form.get('name')||'').trim().slice(0,60),String(form.get('room')||'').trim().slice(0,60),Math.max(1,Math.min(90,Number(form.get('interval'))||7)),form.get('person')==='Sie'?'Sie':'Du',imageKey);
  ...
  if (body.action === 'water' && body.id) await waterPlant(body.id, body.person === 'Sie' ? 'Sie' : 'Du');
  else if (body.action === 'delete' && body.id) await removePlant(body.id);
  else if (body.name?.trim() && body.room?.trim()) await addPlant(body.name.trim().slice(0,60), body.room.trim().slice(0,60), Math.max(1, Math.min(90, Number(body.intervalDays) || 7)), body.person === 'Sie' ? 'Sie' : 'Du');
