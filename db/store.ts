...
function dailyCandidates(dayKey:string) { const seed=Number(dayKey.replaceAll('-','')); const start=seed%gardenPlants.length; return [0,5,9].map((n)=>gardenPlants[(start+n)%gardenPlants.length]); }
