...
export async function getWateringStats() {
  ...
  const todayActivity = await db.prepare(`SELECT COUNT(DISTINCT done_at) AS count FROM (SELECT watered_at AS done_at FROM watering_events UNION ALL SELECT completed_at AS done_at FROM chore_events) WHERE substr(done_at,1,10)=?`).bind(todayKey).first<{count:number}>();
  const todayTasks = Number(todayActivity?.count ?? 0);
...
export async function getGarden() { const db=await ready(); const weekKey=currentWeekKey(); const rows=await db.prepare('SELECT week_key AS weekKey, plant_key AS plantKey, chosen_by AS chosenBy, unlocked_at AS unlockedAt, xp_at_unlock AS xpAtUnlock, room FROM garden_collection ORDER BY unlocked_at').all<{weekKey:string;plantKey:string;chosenBy:string;unlockedAt:string;xpAtUnlock:number;room:string}>(); const xp=await totalHouseXp(db); const collection=rows.results.map((row)=>({...row,plant:gardenPlants.find((p)=>p.key===row.plantKey)??gardenPlants[0]})); let unlockedCount=1; for(let i=0;i<gardenRooms.length-1;i++){if(collection.filter((item)=>item.room===gardenRooms[i]).length>=roomCapacity)unlockedCount=i+2;else break} const rooms=gardenRooms.map((name,index)=>({name,unlocked:index<unlockedCount,count:collection.filter((item)=>item.room===name).length,capacity:roomCapacity})); return {weekKey,xp,candidates:weeklyCandidates(weekKey),collection,rooms,activeRoom:gardenRooms[Math.min(unlockedCount-1,gardenRooms.length-1)],chosenThisWeek:rows.results.some((row)=>row.weekKey===weekKey)}; }
export async function chooseGardenPlant(plantKey:string, person:string, requestedRoom:string) { const db=await ready(); const current=await getGarden(); const weekKey=currentWeekKey(); if(!weeklyCandidates(weekKey).some((p)=>p.key===plantKey)) return current; const room=current.rooms.find((item)=>item.name===requestedRoom&&item.unlocked&&item.count<item.capacity)?.name??current.activeRoom; const xp=await totalHouseXp(db); await db.prepare('INSERT OR IGNORE INTO garden_collection (week_key, plant_key, chosen_by, unlocked_at, xp_at_unlock, room) VALUES (?, ?, ?, ?, ?, ?)').bind(weekKey,plantKey,person,new Date().toISOString(),xp,room).run(); return getGarden(); }
...
export async function completeChore(id:number, person:string, together=false) {
  const db = await ready(); const now = new Date().toISOString(); const today = now.slice(0,10);
  const chore = await db.prepare('SELECT points FROM chores WHERE id = ? AND paused = 0').bind(id).first<{points:number}>();
  if (!chore) return null;
  const activity = await db.prepare(`SELECT COUNT(DISTINCT done_at) AS count FROM (SELECT completed_at AS done_at FROM chore_events UNION ALL SELECT watered_at AS done_at FROM watering_events) WHERE substr(done_at,1,10)=?`).bind(today).first<{count:number}>();
  const bonus = Math.min(10, Number(activity?.count ?? 0) * 2);
